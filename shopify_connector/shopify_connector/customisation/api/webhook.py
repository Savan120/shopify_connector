...
def receive_shopify_order():
    ...
    try:
        ...
        if not shopify_webhook_secret:
            frappe.throw(_("Webhook secret not configured. Please set it up in Shopify Connector Setting."), frappe.ValidationError)
...
def create_payment_entry(si):
    ...
    paid_to = frappe.db.get_all("Account", filters={"account_type": "Cash", "company": si.company},fields= ["name","account_currency"])
...
def customer_creation():
    ...
    if shopify_keys.sync_customer:
        ...
        try:
            ...
            frappe.log_error(f"Failed to parse JSON from request body: {e}", "Shopify Webhook Error")
