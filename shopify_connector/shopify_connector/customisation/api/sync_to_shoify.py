...
def enqueue_send_customer_to_shopify(doc, method):
    if not getattr(doc.flags, "from_shopify", False):
        enqueue(
            "shopify_connector.shopify_connector.customisation.api.sync_to_shoify.send_customer_to_shopify_hook_delayed",
            queue="default",
            timeout=300,
            doc=doc,
...
def send_customer_to_shopify_hook(doc, method):
    ...
    linked_contacts = frappe.db.get_value(
        "Dynamic Link",
    ...
    if linked_contacts:
        try:
            ...
            frappe.log_error(
                f"Error fetching linked contact '{contact_doc_name}' for customer {doc.name}: {str(e)}",
                "Shopify Sync Error",
            )
    ...
    shopify_customer_id = doc.shopify_id or frappe.db.get_value(
        "Customer", doc.name, "shopify_id"
    )
    if shopify_customer_id:
        try:
            ...
            frappe.log_error(
                f"Could not fetch existing Shopify addresses for customer {doc.name}: {str(e)}"
            )
    ...
    try:
        ...
        if shopify_customer_id:
            ...
            response = requests.put(
                url, json=customer_payload, headers=headers, timeout=30
            )
        else:
        ...
        if response.status_code not in (200, 201):
            frappe.log_error(
                f"Shopify customer sync failed: {response.text}", "Shopify Sync Error"
            )
            return
        ...
        frappe.log_error(
            f"Exception during Shopify customer sync: {str(e)}", "Shopify Sync Error"
        )
...
def on_address_update(doc, method):
    ...
    shopify_customer_id = frappe.db.get_value(
        "Customer", {"customer_primary_address": doc.name}, "shopify_id"
    )
    if not shopify_customer_id:
        for row in doc.links:
            if row.link_doctype == "Customer":
                shopify_customer_id = frappe.db.get_value(
                    "Customer", row.link_name, "shopify_id"
                )
                if shopify_customer_id:
                    break
    ...
    if not shopify_customer_id:
        frappe.log_error(
            "Shopify customer ID not found for address update.", "Shopify Sync Error"
        )
        return
    ...
    if not shopify_address_id:
        ...
        frappe.log_error(
            f"Shopify address ID not found for address '{doc.name}'. Cannot update.",
            "Shopify Sync Error",
        )
        return
    ...
    frappe.msgprint(
        f"Shopify address updated successfully: {updated_data['customer_address']['id']}"
    )
...
def send_contact_to_shopify(doc, method):
    ...
    shopify_customer_id = frappe.db.get_value(
        "Customer", {"customer_primary_contact": doc.name}, "shopify_id"
    )
    if not shopify_customer_id:
        for row in doc.links:
            if row.link_doctype == "Customer":
...
def delete_customer_from_shopify(doc, method):
    ...
    if response.status_code != 200:
        frappe.log_error(
            f"Failed to delete customer from Shopify: {response.text}",
            "Shopify Customer Delete Error",
        )
...
def send_item_to_shopify(doc, method):
    ...
    if doc.custom_ignore_product_update:
        ...
        # doc.custom_ignore_product_update = False
        doc.update({"custom_ignore_product_update": False})
        frappe.db.commit()
        return
    ...
    if parent_doc_for_payload.image:
        ...
        parent_doc_for_payload.has_variants
        or parent_doc_for_payload.name != item_triggering_sync.name
    ):
        variants = frappe.get_all(
            "Item",
            filters={"variant_of": parent_doc_for_payload.name},
            fields=["image"],
        )
        for variant in variants:
            if variant.image:
                image_url = (
                    ...
                    if not variant.image.startswith("http")
                    else variant.image
                )
                break
    ...
    product_payload = {
        "product": {
            ...
            "sku": (
                parent_doc_for_payload.item_code
                if not parent_doc_for_payload.has_variants
                else None
            ),
            "variants": [],
            "images": [],
    ...
    if frappe_template_item_name:
        ...
        if product_shopify_id_to_update:
            ...
            try:
                ...
                if get_response.status_code == 200:
                    ...
                    frappe.log_error(
                        f"Failed to fetch existing Shopify product {product_shopify_id_to_update}: Status {get_response.status_code} - {get_response.text}",
                        "Shopify Sync Error",
                    )
            except requests.exceptions.RequestException as e:
                frappe.log_error(
                    f"Failed to fetch existing Shopify product {product_shopify_id_to_update}: {e}",
                    "Shopify Sync Error",
                )
        ...
        for variant_frappe in all_variant_items_from_frappe:
            ...
            if variant_frappe.get("custom_variant_id"):
                ...
                variant_data["id"] = existing_shopify_variants_map[
                    variant_frappe["item_code"]
                ]["id"]
                frappe.db.set_value(
                    "Item",
                    variant_frappe["name"],
            ...
            if variant_doc.image and variant_doc.custom_variant_id:
                ...
                if {"src": variant_image_url} not in images_to_send_to_shopify and {
                    "src": variant_image_url
                } not in product_payload["product"]["images"]:
                    images_to_send_to_shopify.append(
                        {
                            "src": variant_image_url,
        ...
        for attr in attribute_order:
            if option_map[attr]:
                product_payload["product"]["options"].append(
                    {"name": attr, "values": list(option_map[attr])}
                )
            elif (
                product_shopify_id_to_update
                and existing_shopify_product
                and "options" in existing_shopify_product
            ):
                for existing_option in existing_shopify_product["options"]:
                    if existing_option["name"] == attr:
                        if existing_option.get("values"):
                            ...
                            product_payload["product"]["options"].append(
                                {"name": existing_option["name"], "values": []}
                            )
    ...
    if (
        parent_doc_for_payload.has_variants
        and not product_payload["product"]["variants"]
    ):
        return
    ...
    if response.status_code in [200, 201]:
        ...
        if frappe_template_item_name:
            frappe.db.set_value(
                "Item", frappe_template_item_name, "shopify_id", shopify_product["id"]
            )
            frappe.db.set_value(
                "Item", frappe_template_item_name, "custom_send_to_shopify", 1
            )
        else:
            frappe.db.set_value(
                "Item", item_triggering_sync.name, "shopify_id", shopify_product["id"]
            )
            frappe.db.set_value(
                "Item", item_triggering_sync.name, "custom_send_to_shopify", 1
            )
        ...
        for variant in shopify_product.get("variants", []):
            ...
            if sku in sent_skus and variant_id:
                frappe_variant_item_name = frappe.db.get_value(
                    "Item", {"item_code": sku}, "name"
                )
                if frappe_variant_item_name:
                    try:
                        frappe.db.set_value(
                            "Item",
                            frappe_variant_item_name,
                            {
                        ...
                        frappe.log_error(
                            f"Failed to set variant/inventory ID for SKU {sku} (Frappe item: {frappe_variant_item_name}): {str(e)}",
                            "Shopify Sync",
                        )
        doc.flags.from_shopify = True
        frappe.db.commit()
        ...
        frappe.log_error(
            title=f"Failed to sync product {parent_doc_for_payload.name} to Shopify",
...
def shopify_credentials():
    ...
    return {"access_token": access_token, "shopify_graph_url": shopify_graph_url}
...
def create_shopify_draft_order(doc, method):
    ...
    customer_email = (
        frappe.db.get_value("Contact", contact[0].parent, "email_id")
        if contact
        else None
    )
    ...
    for item in doc.items:
        ...
        if items.custom_variant_id:
            line_item["variantId"] = (
                f"gid://shopify/ProductVariant/{items.custom_variant_id}"
            )
        else:
    ...
    if doc.discount_amount or doc.additional_discount_percentage:
        applied_discount = {
            ...
            "valueType": (
                "PERCENTAGE" if doc.additional_discount_percentage else "FIXED_AMOUNT"
            ),
            "title": "ERP Discount",
        }
    ...
    variables = {
        "input": {
            ...
            "tags": [doc.status],
            "taxExempt": False,
            "shippingLine": {"title": "Standard Shipping", "price": float(0)},
    ...
    draft_order = (
        response.json().get("data", {}).get("draftOrderCreate", {}).get("draftOrder")
    )
