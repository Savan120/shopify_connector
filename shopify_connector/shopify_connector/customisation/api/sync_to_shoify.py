...
#     item_to_sync = frappe.get_doc("Item", doc.name)
#     frappe_template_item_name = None
#     product_shopify_id_to_update = None
#     parent_doc_for_payload = None

#     if item_to_sync.variant_of:
#         frappe_template_item_name = item_to_sync.variant_of
...
#     elif item_to_sync.has_variants:
#         frappe_template_item_name = item_to_sync.name
#         parent_doc_for_payload = item_to_sync
#         product_shopify_id_to_update = item_to_sync.shopify_id
#     else:
#         parent_doc_for_payload = item_to_sync
#         product_shopify_id_to_update = item_to_sync.shopify_id
...
#         print(f"Skipping sync for {parent_doc_for_payload.name}: custom_send_to_shopify is not checked.")
#         return
...
#     elif not item_to_sync.has_variants:
#         product_payload = {
#             "product": {
#                 "title": item_to_sync.item_name,
#                 "body_html": f"<strong>{item_to_sync.description or ''}</strong>",
#                 "vendor": item_to_sync.brand or "Default Vendor",
#                 "product_type": item_to_sync.item_group or "",
#                 "sku": item_to_sync.item_code,
#                 "variants": [],
#                 "images": [],
#             }
#         }
#         variant_data = {
#             "price": item_to_sync.shopify_selling_rate or 0.0,
#             "sku": item_to_sync.item_code,
#             "inventory_policy": "deny",
#             "taxable": True,
#         }
#         if item_to_sync.custom_variant_id:
#             variant_data["id"] = item_to_sync.custom_variant_id
...
#         else:
#             frappe.db.set_value("Item", item_to_sync.name, "shopify_id", shopify_product["id"])
...
def send_item_to_shopify(doc, method):
    ...
    frappe_template_item_name = None
    parent_doc_for_payload = None # This will be the actual document whose details populate the product payload
    ...
    # Determine product_shopify_id_to_update from the parent/template item
    product_shopify_id_to_update = parent_doc_for_payload.shopify_id
    ...
    # Initial product payload, using parent_doc_for_payload for core details
    product_payload = {
        "product": {
            "title": parent_doc_for_payload.item_name,
            "body_html": f"<strong>{parent_doc_for_payload.description or ''}</strong>",
    ...
    if frappe_template_item_name: # This means it's a template or a variant of a template
        # Get all potential variants related to this template
        all_variant_items_from_frappe = frappe.get_all(
            "Item",
            filters={"variant_of": frappe_template_item_name},
            fields=["name", "item_code", "shopify_selling_rate", "image", "custom_variant_id", "send_to_shopify"] # Added send_to_shopify
        )
        ...
        position_counter = 1 # To ensure unique positions for variants

        for variant_frappe in all_variant_items_from_frappe:
            # --- NEW LOGIC: ONLY PROCESS VARIANT IF ITS send_to_shopify IS CHECKED ---
            if not variant_frappe.get("send_to_shopify"):
                print(f"Skipping variant '{variant_frappe['name']}' because send_to_shopify is not checked.")
                # If a variant is unchecked, and it exists on Shopify, we might want to delete it from Shopify.
                # This logic is more complex and usually handled with a separate deletion process or by marking it inactive.
                # For now, we'll just not include it in the payload.
                continue
            ...
            values = [attr_dict.get(attr) for attr in attribute_order]
            values += [None] * (3 - len(values)) # Shopify supports up to 3 options
            variant_data = {
                "option1": values[0],
                "option2": values[1],
                "option3": values[2],
                ...
                "taxable": True,
                "position": position_counter, # Use a counter for position
            }
            position_counter += 1

            # Try to get existing Shopify variant ID
            if variant_frappe.get("custom_variant_id"):
                variant_data["id"] = variant_frappe["custom_variant_id"]
            elif variant_frappe["item_code"] in existing_shopify_variants_map:
                variant_data["id"] = existing_shopify_variants_map[variant_frappe["item_code"]]["id"]
                # Update custom_variant_id on the Frappe item for future reference
                frappe.db.set_value("Item", variant_frappe["name"], "custom_variant_id", variant_data["id"])
        ...
        for attr in attribute_order:
            ...
            # Ensure all 3 options are sent if they exist on Shopify, even if no current Frappe variant uses them
            elif product_shopify_id_to_update and existing_shopify_product and "options" in existing_shopify_product:
                for existing_option in existing_shopify_product["options"]:
                    if existing_option["name"] == attr:
                        # Ensure values are included if they existed previously
                        if existing_option.get("values"):
                             product_payload["product"]["options"].append(existing_option)
                        else:
                            # If no values, just send the name to avoid issues
                            product_payload["product"]["options"].append({"name": existing_option["name"], "values": []})


    elif not item_triggering_sync.has_variants: # This handles simple items without variants
        # Check if the simple item itself has send_to_shopify checked
        if not item_triggering_sync.send_to_shopify:
            print(f"Skipping sync for simple item '{item_triggering_sync.name}': send_to_shopify is not checked.")
            return
        ...
        if image_url: # Use image_url from the simple item
            product_payload["product"]["images"].append({"src": image_url})

    # --- API Calls ---
    # Only make API call if there are variants to send (for template products)
    # or if it's a simple product with its checkbox checked.
    if parent_doc_for_payload.has_variants and not product_payload["product"]["variants"]:
        print(f"Skipping API call for '{parent_doc_for_payload.name}' as no variants are marked for sync.")
        # Optional: Logic to delete the product from Shopify if no variants are selected
        # or if it exists on Shopify but now has no active variants. This would be a separate DELETE call.
        return
    ...
    if product_shopify_id_to_update:
        ...
        # If it's a new product, but no variants are selected, this POST call might create an empty product.
        # The check above `if not product_payload["product"]["variants"]` helps prevent this.
        url = f"https://{SHOPIFY_API_KEY}:{SHOPIFY_ACCESS_TOKEN}@{SHOPIFY_STORE_URL}/admin/api/{SHOPIFY_API_VERSION}/products.json"
        response = requests.post(url, json=product_payload, verify=False)

    if response.status_code in [200, 201]:
        shopify_product = response.json()["product"]
        # Set the shopify_id on the *parent/template* item
        if frappe_template_item_name:
            frappe.db.set_value("Item", frappe_template_item_name, "shopify_id", shopify_product["id"])
            frappe.db.set_value("Item", frappe_template_item_name, "custom_send_to_shopify", 1) # Ensure parent remains checked if sync was successful
        else:
            frappe.db.set_value("Item", item_triggering_sync.name, "shopify_id", shopify_product["id"])
            frappe.db.set_value("Item", item_triggering_sync.name, "send_to_shopify", 1) # Ensure simple item remains checked

        # Update variant IDs on Frappe variants that were sent
        ...
        for variant in shopify_product.get("variants", []):
            ...
            if sku in sent_skus and variant_id: # Only update if this variant was part of the payload we sent
                frappe_variant_item_name = frappe.db.get_value("Item", {"item_code": sku}, "name")
                if frappe_variant_item_name:
                    try:
                        frappe.db.set_value("Item", frappe_variant_item_name, {
